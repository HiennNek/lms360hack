!function (a) {
    ...
    b.prototype.update = function () {
        ...
        for (var a = 0; a < this.o.length; a++) {
            this.o[a].update();
            this.o[a].h();
            for (var bIdx = this.o.length - 1; bIdx > a; bIdx--) {
                var cDist = Math.sqrt(
                    Math.pow(this.o[a].x - this.o[bIdx].x, 2) +
                    Math.pow(this.o[a].y - this.o[bIdx].y, 2)
                );
                if (cDist > 120) continue;
                this.g.beginPath();
                this.g.strokeStyle = this.options.particleColor;
                this.g.globalAlpha = (120 - cDist) / 120;
                this.g.lineWidth = .7;
                this.g.moveTo(this.o[a].x, this.o[a].y);
                this.g.lineTo(this.o[bIdx].x, this.o[bIdx].y);
                this.g.stroke();
            }
        }
